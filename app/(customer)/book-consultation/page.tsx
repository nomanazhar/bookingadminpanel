...
export default async function BookConsultationPage() {
  ...
  if (authUser) {
    const orders = await getOrdersByCustomer(supabase, authUser.id);

    if (orders && orders.length) {
      const now = new Date();
      const candidates = orders.filter((o: any) => {
        const dt = parseBookingDateTime(
          o.booking_date,
          o.booking_time || "00:00:00"
        );
        return (
          (o.status === "pending" || o.status === "confirmed") && dt >= now
        );
      });
      candidates.sort((a: any, b: any) => {
        const ad = parseBookingDateTime(
          a.booking_date,
          a.booking_time || "00:00:00"
        );
        const bd = parseBookingDateTime(
          b.booking_date,
          ...
          <div className="bg-muted rounded-xl shadow divide-y">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-muted-foreground text-sm">
                  Browse and book available treatments
                </div>
              </div>
              <ChevronRight className="w-5 h-5 text-muted-foreground" />
            </Link>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-muted-foreground text-sm">
                  Manage recent Bookings and view previous purchases
                </div>
              </div>
              <ChevronRight className="w-5 h-5 text-muted-foreground" />
            </Link>
