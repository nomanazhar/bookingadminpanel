...
-- FIXES APPLIED:
-- • Removed debug RLS policy (security hole)
-- • Moved pg_cron schedule AFTER function definition
-- • Added GRANT EXECUTE on create_customer_order_with_sessions for authenticated
-- • Added role guard on get_dashboard_data (admin only)
-- • Added SECURITY INVOKER on orders_with_sessions view (respects RLS)
-- • Added updated_at triggers on all tables
-- • Added missing indexes: sessions(status), orders(service_id)
-- • Fixed materialized view unique index placement (before cron schedule)
...
-- FIX: removed the "Allow all updates for debugging" policy entirely (was a critical security hole)
...
-- FIX: explicit UPDATE policy so authenticated users can update only their own profile
...
-- ============================================
-- STEP 15: UPDATED_AT TRIGGERS (all tables)
-- FIX: original schema only had this on profiles
-- ============================================
...
-- ============================================
-- STEP 16: ORDERS WITH SESSIONS VIEW
-- FIX: added WITH (security_invoker = true) so the view
--      respects the caller's RLS permissions instead of bypassing them
-- ============================================
...
-- ============================================
-- STEP 17: DASHBOARD MATERIALIZED VIEW + STATS
-- FIX: unique index created immediately after the view
--      (before any CONCURRENTLY refresh is attempted)
-- ============================================
...
-- FIX: index created right here, before refresh_dashboard_stats is defined/called
CREATE UNIQUE INDEX IF NOT EXISTS idx_dashboard_stats_unique
ON public.dashboard_stats((1));

-- ============================================
-- STEP 18: REFRESH FUNCTION
-- (defined AFTER the materialized view + its unique index)
-- ============================================
...
-- ============================================
-- STEP 19: GET DASHBOARD DATA RPC
-- FIX: added admin-only guard so arbitrary authenticated users cannot call it
-- ============================================
...
BEGIN
  -- Access control note: no is_admin() guard here because this function is
  -- SECURITY DEFINER and called exclusively via the service role key from
  -- server-side admin code. Under the service role context, auth.uid() returns
  -- NULL so is_admin() always returns false and would block legitimate calls.
  -- Security is enforced at the application layer (requireAdmin() in auth.ts)
  -- and by keeping the service role key server-side only, never in the browser.
  ...
  INTO
    ...
    v_total_services,
    v_total_doctors,
    v_future_appointments
  FROM public.dashboard_stats
  LIMIT 1;

  -- Fallback: if materialized view is empty, compute once on-demand
  IF NOT FOUND THEN
...
-- ============================================
-- STEP 20: CUSTOMER ORDER CREATION RPC
-- FIX: added GRANT EXECUTE for authenticated role
--      (original schema forgot this, so customers could never call it)
-- ============================================

CREATE OR REPLACE FUNCTION public.create_customer_order_with_sessions(
  -- Required params first (no defaults) — PostgreSQL requires this ordering
  p_service_id        uuid,
  p_booking_date      text,
  p_booking_time      text,
  -- Optional params with defaults follow
...
BEGIN
  ...
  v_session_count := COALESCE(p_sessions, p_session_count, 1);
  v_session_count := GREATEST(1, LEAST(10, v_session_count));

  IF p_discount_percent IS NOT NULL THEN
    v_discount_percent := p_discount_percent;
  ELSIF v_session_count > 1 THEN
    IF    v_session_count = 3  THEN v_discount_percent := 25;
  ...
  ELSE
    v_unit_price := ROUND(v_base_price * (1 - v_discount_percent / 100.0), 2);
  END IF;

  IF p_total_amount IS NOT NULL THEN
    v_total_amount := p_total_amount;
  ELSE
    v_total_amount := v_unit_price * v_session_count;
  END IF;
  ...
  VALUES (
    ...
    COALESCE(TRIM(COALESCE(v_profile.first_name, '') || ' ' || COALESCE(v_profile.last_name, '')), ''),
...
-- FIX: authenticated users must be able to call this function to place orders
-- Type list matches corrected param order: service_id, booking_date, booking_time, then optionals
...
-- Categories
CREATE INDEX IF NOT EXISTS idx_categories_is_active_display_order
...
CREATE INDEX IF NOT EXISTS idx_services_is_popular_is_active
  ON public.services(is_popular, is_active);
...
CREATE INDEX IF NOT EXISTS idx_orders_customer_email_date_time
  ON public.orders(customer_email, booking_date, booking_time);
-- FIX: added missing index on service_id (used in joins throughout)
...
CREATE INDEX IF NOT EXISTS idx_sessions_order_id_session_number
  ON public.sessions(order_id, session_number);
-- FIX: added missing index on status (frequently filtered in queries)
CREATE INDEX IF NOT EXISTS idx_sessions_status                 ON public.sessions(status);

-- ============================================
-- STEP 22: PG_CRON SCHEDULE
-- FIX: moved to end of file, AFTER refresh_dashboard_stats() is defined
--      (original had this before the function existed)
-- ============================================
